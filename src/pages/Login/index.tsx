  ...
  class Login extends Component < FormProps, {} > {
    ...
    handleSubmit = (e : React.FormEvent) => {
      ...
      const callback = (res : any) => {
        if (res.success && res.data) {
          ...
          reloadAuthorized();
          sessionStorage.setItem('sysUserId', res.data.sysUserId)
          sessionStorage.setItem('userName', res.data.companyName)
          dispatch(routerRedux.push({pathname: '/userInfo/list'}));
    ...
    setLeftTime() {
      ...
      timer = setInterval(() => {
        timeLeft--
        // 格式化剩余时间
        this.setState({getCodeText: `${timeLeft}s重新发送`})
        if (timeLeft === 0) {
          clearInterval(timer)
          this.setState({timeLeft: 60, disabled: false, getCodeText: '发送验证码'})
        }
      }, 1000);
    }
